...
): Promise<Types.GcmCheckinResponse> {
  const body = await (
    await request(CHECKIN_URL, {
      method: "POST",
      headers: {
...
async function doRegister(
  { androidId, securityToken }: Types.GcmCheckinResponse,
  config: Types.ClientConfig,
): Promise<Types.GcmData> {
...
function prepareCheckinBuffer(config: Types.ClientConfig) {
  ...
  const payload: checkin_proto.IAndroidCheckinRequest = {
    ...
    otaCert: [],
    securityToken: gcm?.securityToken
      ? Long.fromString(gcm?.securityToken, true)
      : undefined,
