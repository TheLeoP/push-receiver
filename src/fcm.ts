...
export async function installFCM(
  config: Types.ClientConfig,
): Promise<Types.InstallationData> {
  const response = await request(
    getEndpoint(config, FCM_INSTALLATION, "installations"),
    {
      method: "POST",
      headers: {
        "x-firebase-client": getEmptyHeatbeat(),
...
export async function registerFCM(
  gcmData: Types.GcmData,
  installation: Types.InstallationData,
  keys: Types.Keys,
  config: Types.ClientConfig,
): Promise<Types.FcmRegistrationResponse> {
  ...
  const response = await request(
    getEndpoint(config, FCM_REGISTRATION, "registrations"),
    requestOptions,
  );
